...
interface MagentoProductsRestResponse {
  items?: {
    sku: string;
  }[];
}
...
export async function fetchStockForSkus(
  skus: string[]
): Promise<Record<string, SkuStockInfo>> {
  if (!skus.length) return {};

  if (!PRODUCTS_URL || !SALABLE_QTY_URL_BASE || !MAGENTO_API_TOKEN) {
    throw new Error(
      'PRODUCTS_URL, SALABLE_QTY_URL_BASE or MAGENTO_API_TOKEN is not set; cannot validate stock.'
    );
  }
  ...
  if (!prodRes.ok) {
    ...
    throw new Error(
      `Magento REST products query failed: ${prodRes.status} ${prodRes.statusText}`
    );
  }

  let prodJson: MagentoProductsRestResponse;
  try {
    prodJson = JSON.parse(prodText) as MagentoProductsRestResponse;
  } catch (e) {
  ...
  const result: Record<string, SkuStockInfo> = {};
  ...
  for (const sku of uniqueSkus) {
    ...
    const salableUrl = `${SALABLE_QTY_URL_BASE}/${encodeURIComponent(
      sku
    )}/${MAGENTO_STOCK_ID}`;

    try {
      const salableRes = await fetch(salableUrl, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${MAGENTO_API_TOKEN}`,
      ...
      const salableQty =
        Number.isFinite(salableQtyNumber) && salableQtyNumber > 0
          ? salableQtyNumber
          : 0;
